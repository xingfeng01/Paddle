...
#pragma unroll
    for (int it = 0; it < kIterationsV; ++it) {
      const VecT* softmaxptr_v =
          reinterpret_cast<const VecT*>(&softmax[(first_batch + i) * dim]);
      const VecT* labelsptr_v =
          reinterpret_cast<const VecT*>(&labels[(first_batch + i) * dim]);
